<template>
    <base-container boxed with-loader :is-loading="isLoading" :loader-color="vars.loaderColor" id="printable">
        <form @submit.prevent="submit">
            ...
            <search-tag v-model="formData.emailsArray" :label="formLabels.emails" :options="preRequisite.emails" @tag="addEmails" :error.sync="formErrors.emails" input-classes="mb-0" />
    ...
    export default {
        ...
        methods: {
            addEmails(email) {
                ...
                const already = this.preRequisite.emails.find(e => e.slug === tag.slug)
                if(already) {
                    return
                }
            ...
            beforeSubmit() {
                if(this.formData.emailsArray && this.formData.emailsArray.length > 0) {
                    this.formData.emails = this.formData.emailsArray.map(e => e.slug)
                }
            },
        },
        mounted() {
