...
class ConfigRepository
{
    ...
    public function getConfig(): array
    {
        ...
        $app_config['signal_servers'] = $app_config['signal'];
    ...
    private function getPublicConfig($db_config = array(), $authenticated = false, $two_factor = false): array
    {
        ...
        if (request('meeting') && request()->boolean('pam')) {
            ...
            if ($meeting && $meeting->getMeta('is_pam') && $meeting->getMeta('status') !== MeetingStatus::CANCELLED && $meeting->getMeta('status') !== MeetingStatus::ENDED) {
                ...
                $configs['signal_servers'] = Arr::get($db_config, 'signal', []);
    ...
    public function store(): void
    {
        ...
        $method = 'set' . Str::studly(request('type')) . 'Config';
     ...
     */
    private function setPaymentGatewayConfig()
    {
        $this->storeConfig();
    }
     ...
     */
    private function setMembershipConfig()
    {
        request()->validate([
            'monthly_charge' => 'required_if:enabled,1|min:0|numeric',
            'annually_charge' => 'required_if:enabled,1|min:0|numeric',
            'lifetime_charge' => 'required_if:enable_lifetime_membership,1|min:0|numeric',
        ]);
    ...
    /**
     * Google Api Key Json file Uploading.
     */
    public function startImport() : array
    {
        ...
        if ($extension != 'json') {
            throw ValidationException::withMessages(['message' => trans('config.system.import.json_file_supported')]);
        }

        $uuid = request()->file('file')->getClientOriginalName();
        
        $isExists = \Storage::exists(public_path($this->path.$uuid));
        
        if ($isExists) {
            \Storage::delete(public_path($this->path.$uuid));
        }
        
		\Storage::putFileAs($this->path, request()->file('file'), $uuid);
     ...
     * Get Translator
     */
    public function getTranslator($uuid): bool
    {
        $config = $this->config->firstOrCreate(['name' => 'system']);
        $value = $config->value;
        if ($uuid === 'microsoft_azure') {
            if ($value["azure_apikey"]) {
                return true;
            } else {
                return false;
            }
        } else if ($uuid === 'google_translate') {
            if ($value["google_apikey"]) {
                return true;
            } else {
                return false;
            }
    ...
    /**
     * Save Microsoft AzureApiKey
     */
    public function saveAzureApiKey($azure_api_key) : void
    {
        $config = $this->config->firstOrCreate(['name' => 'system']);
        $value = $config->value;
        $value["azure_apikey"] = $azure_api_key;
        if (is_null($azure_api_key)) {
            unset($value["azure_apikey"]);
        }
        $config->value = $value;
        $config->save();
    }

    public function saveGoogleApiKey($google_api_key) : void
    {
        $config = $this->config->firstOrCreate(['name' => 'system']);
        $value = $config->value;
        $value["google_apikey"] = $google_api_key;
        if (is_null($google_api_key)) {
            unset($value["google_apikey"]);
        }
        $config->value = $value;
        $config->save();
    }
