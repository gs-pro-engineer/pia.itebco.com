...
class UserRepository
{
    ...
    public function getPreRequisite() : array
    {
        ...
        $languages = ArrHelper::getTransList('languages');

        return compact('genders', 'roles', 'languages');
    }
    ...
    private function formatParams($uuid = null) : array
    {
        ...
        $language = Arr::get(request('profile.language', []), 'uuid');
        
        if (! in_array($language, ArrHelper::getList('languages'))) {
            throw ValidationException::withMessages(['profile.language' => trans('global.could_not_find', ['attribute' => trans('user.props.language')])]);
        }
